...
namespace ProjectListFApp
{
    public class AuthenticationManager : IDisposable
    {
        ...
        private const string defaultAADAppId = "<Application ID>";
        ...
        public ProjectContext GetContext(Uri web, string userPrincipalName, string userPassword)
        {
            SecureString securePassword = new SecureString();

            foreach (char c in userPassword.ToCharArray()) securePassword.AppendChar(c);

            var context = new ProjectContext(web.AbsoluteUri);

            context.ExecutingWebRequest += (sender, e) =>
            {
                string accessToken = EnsureAccessTokenAsync(new Uri($"{web.Scheme}://{web.DnsSafeHost}"), userPrincipalName, new System.Net.NetworkCredential(string.Empty, securePassword).Password).GetAwaiter().GetResult();
